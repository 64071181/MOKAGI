...
# ================== 模型設定 ===================
MODEL_NAME="huihui_ai/qwen3-abliterated:1.7b" # 這裡指定 Ollama 模型名稱，可在 https://ollama.com/huihui_ai/qwen3-abliterated:1.7b 直接找其他模型
...
echo -e "${GREEN}==========================================${NC}"
echo -e "${GREEN}  [3/7] 下載模型 ${MODEL_NAME} ${NC}"
echo -e "${GREEN}==========================================${NC}"
# 定義環境變數以便在當前會話執行指令
export OLLAMA_HOST="[::]:11434"
# --- [下載 + 路徑檢查] ---
if ollama list | grep -q "^${MODEL_NAME} "; then
    echo -e "${YELLOW}模型 ${MODEL_NAME} 已存在，跳過下載。${NC}"
else
    echo "正在從 Ollama 庫拉取模型..."
    if ! ollama pull ${MODEL_NAME}; then
        echo ""
        echo "❌ 錯誤:模型路徑 [${MODEL_NAME}] 似乎不存在！"
        echo "------------------------------------------------"
        echo "這通常是因為:"
        ...
        echo "   或將指令碼開頭的 ${MODEL_NAME} 修改為官方版本 (如: qwen2.5:3b)。"
        echo ""
        echo "-- 終止指令碼，防止後續指令崩潰 --"
        echo "------------------------------------------------"
        exit 1
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for cmd, handler in cmd_map.items():
        if user_text == cmd or user_text.startswith(cmd + " "):
            ...
            try:
                result = handler(args, str(chat_id)) if handler else "外掛處理函式未定義。"
                ...
                await update.message.reply_text(f"❌ 外掛執行錯誤: {e}")
            return
